...
- # 分布式系统的计算切片
	- 一个计算切片是相对于一个全局谓词定义的
	- 非形式化地说，一个切片是定义在一个事件子集上的偏序关系，子集中的事件必须同时执行
...
- # 获取分布式计算切片的算法
	- ## 形式化描述
		- **输入**：一个计算$(E,\rightarrow)$和一个正则表达式$B$
		- **输出**：一个计算切片$(I_B,F,\rightarrow_B)$，此计算切片满足以下性质：
			- $F$的任何一致分割仍然是$E$的一个一致分割
			- $F$的任何一致分割都满足$B$
			- $E$的所有满足$B$的一致分割也是$F$的一致分割
		- **第一步**
			- 使用同一团队(应该)之前的[算法](file:///C:/Users/samsung/Downloads/s004460050049.pdf)计算出一个$E$中满足$B$的最小一致分割$V$
			- 如果不存在这么一个分割，那么计算切片不存在
		- **第二步**
			- 同样，用之前的算法计算出$E$的满足$B$的最大一致分割$W$
		- **第三步**
			- 对于$W-V$中的每一个事件$e$，计算$E$中包含$e$且满足$B$的一致分割$J(e)$
		- **第四步**
			- 令一个**相等关系**$R$：$(e,f)\in R\equiv J(e)=J(f)$
			- 由$R$确立的所有**equivalence class**记为$C_i,1\le i\le m$
			- 输出结果为：
				- $F:=\{C_i\ |\ 1\le i\ le m\}$
				- $\rightarrow_B := \{(C_i,C_j)\ |\ J(C_i) \subsetneq J(C_j)\}$
			- 其中$J(C_i)$等于$J(e)$，$e$是$C_i$中某个事件