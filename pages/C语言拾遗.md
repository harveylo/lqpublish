...
- # 宏
	- ## 预定义宏
		- 很多库文件中都有已经定义好的宏
		- 编译器在编译时也会有事先定义好的宏，可以直接使用
		- ``__LINE__``
			- 当前行的行数
	- ## 宏函数
		- 宏也可以用于定义函数
		- 形如：``#define FUN(x) x+2``
			- 实际执行时，``FUN(2)``会被展开为``2+2``
		- **[[$red]]==只做替换，不做计算！==**
		- 计算类宏函数的函数体最好用括号括起来，保证使用时的正确性
		- 在参数前加上``#``表示让该参数变为字符串
		- 需要换行时使用``\``
		- 可以嵌套定义传入一个宏函数的宏函数
	- ## 引用(#include)
		- 实际上，#include可以出现在任何地方
		- 预处理的时候，会递归地对所有include进行复制粘贴，即，将被include的文件中的所有内容粘贴到include语句的位置
		- **双引号**
			- 搜寻当前目录，如果没有则
			- 搜寻系统目录
		- **尖括号**
			- 搜寻系统目录
		- 系统的库目录可以在环境变量里指定
		- 总的来说，双引号的搜索范围要大于尖括号，使用双引号更能保证正确性
		- 使用gcc编译时，使用``-I``可以增添搜索的系统目录
	- ## 定义与展开
		- ``#define`` 所定义的宏可以看作一种变量，程序可以引用，后续的宏也可以引用
			- 有点类似sh脚本中的变量，不经定义就可以使用
			- 所以`#if AA == BB` 若AA和BB事先没有定义，则判断结果永远位true
		- ``##``可以用来将两个宏的类容拼接起来
			- ``#define A sys ## tem``，则A等于system
		- 在展开时，宏会一直嵌套展开知道没有宏可供展开为止
	- ## 宏的优缺点
		- ### 优点
			- 提供灵活的用法
			- 接近自然语言的写法
	...
	- **static**
		...
		- 如果是**局部**静态变量
			...
			- 在c中只能使用常数来初始化静态变量，不能用某个函数的返回值初始化静态变量
				- 但是在C++中可以
		- 结构体中不应当有单独的静态成员
			- 结构体中的所有成员应该在同一个内存段，因为c中结构体的地址计算就是通过起始地址加上偏移来计算成员地址的
...
- # 语法
	- ## desinator initialization
		- 一个数组在声明且初始化的时候可以显示地通过小标来给出初始值
		- ``int a[3] = {[1]=4};``
	- ## 位域
		- 在一个结构体中指定某一成员所占地bit位数
		- [菜鸟教程-位域](https://www.runoob.com/cprogramming/c-bit-fields.html)
	- ## 共用体(union)
		- 对同一片内存空间可以做多种不同解释
		- [菜鸟教程-共用体](https://www.runoob.com/cprogramming/c-unions.html)
- # 概念
	- **Translation Unit**
	...
	- **可重入(re-entrant)函数**
		...
		- 中断处理程序要求是可重入的
	- **Wrap around**
		- 溢出时的一种处理方式，即如果比类型最大可能值还大，那么从最小可能值开始；如果比最小可能值还小，那么从最小大可能值开始
		- 关于c语言中何时产生wrap around的详细描述：[wrap around in c](https://stackoverflow.com/questions/19842215/wrap-around-explanation-for-signed-and-unsigned-variables-in-c)
		- 在C中，有符号类型溢出时会导致undefined behavior(UB)而不是wrap around
			- 所以要多使用unsigned避免潜在的UB
			- ``-fwrapv``可以强制有符号整数溢出为wraparound
		- uint8_t == unsigned char
- # 表达式
	- ## 逗号表达式
		- 依次计算，将最后一个语句的结果作为结果返回
		- ``z = ++a,++b``
			- z=b+1
		-
- # 杂项
	- ## 为什么要在头文件中声明，在源文件中定义
		- c是**[[$red]]==使用前声明==**的，意味着哪怕一个函数或变量在别的文件中已经定义，在另一文件中使用前也要先声明，告诉编译器去实际定义处寻找该函数
		- 因此将声明置于头文件中可以方便其他文件地引用
			- 如果一个函数需要在很多地方都用到，写进头文件里就能避免在各个文件中使用前都先写一行声明代码
	- ## 避免头文件重复引用导致的重复声明和循环引入
		- 注意引用顺序
		- 引入[[$red]]==**include guard**==：
			- ```
			  ...
			  #endif /* GRANDPARENT_H */
			  ```
			- include guard 就是如上的宏，如果已经被引用一次，则相关的宏会被定义，再次被引用时不会再次进行定义
	- ## 头文件中可以放哪些内容
	...
	- ## 代码可读性（可维护性）
		- ### 螺旋法则（clockwise/spiral rule）
			- **仅在对付一些不可读代码时可以使用的方法，自己写代码时并不建议把代码写成需要这样才能看懂的地步**
		...
		- ### 可读性（可维护性）建议
			...
			- **使用预编译宏控制代码生成**
				...
				- [[$blue]]==缺点==
					...
					- 给IDE解析带来困难
			- **善用struct/union**
				- 对于复杂的情况，struct/unioin是更好的设计
- # 函数
	- ## 本质概念
		- 实际上就是一堆可执行的指令
		- 被载入进内存之后，其第一条指令的地址就是执行该函数的入口
		- 因此在内存中的函数是可以被拷贝到任何地方然后执行的
		- 调用某个函数就是跳转到该函数第一条机器码的过程
			- 这些机器码包括了参数传递处理的指令
		- 参数传递是通过一些列的寄存器和内存值传递来完成的（即不停地读入和写出）
- # 指针
	- ## 本质概念
		- 也是一个变量，不过存放的是**首地址**，自身也存放在内存中，自己也有一个内存地址
		- 因此不论任何类型，甚至是自定义结构体的指针，在32位系统下，都是一个四字节32位的值，指向内存中的一个地址
	- 任何指针都是一个**地址**，只不过其类型确定了如何对这个地址**做解读**
	- **[[$red]]==任何==**东西都可以被取地址，函数也不例外
- # 内存
	- [Linux下32位程序的典型布局](https://www.cnblogs.com/zjuhaohaoxuexi/p/16210498.html)
		- ![](https://img2022.cnblogs.com/blog/1852906/202204/1852906-20220430161404050-235745048.jpg)
	...
	- ## 堆
		...
		- 使用malloc和free来管理
		- 一般来说程序内存空间中的所有未使用空间都可以是堆（除去栈的最大大小）
