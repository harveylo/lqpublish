- [[在WSL中安装OpenGL开发环境]]
- [[在windows中利用msys2和vscode快捷配置opengl开发环境]]
- [[GLSL]]
- # 简单理解OpenGL
  collapsed:: true
...
- # Texture
	- ## Texture Coordinates
	  collapsed:: true
		...
		- 贴图坐标的范围都在**[[$red]]==[0,1]==**之间，对于超出这个范围的贴图坐标，opengl一般有如下处理方式：
			...
			- 这种处理范围外贴图坐标的方式叫做：**[[$red]]==Texture Warpping==**
		- 上述四种处理方式的效果如图所示：
	...
	- ## Texture Unit（贴图单元）
		- opengl一次性可以设置多个贴图，不同的贴图通过不同的**Texture Unit**来区分
			- Texture Unit可以被看作指向某个贴图的地址
			- Texture Unit是一个整数值
		- 默认的Texture Unit为0，同时也是默认激活(Active)贴图单元
			- 但**不是所有图形驱动都会设置默认贴图单元**
		- 可以同时绑定和激活多个贴图单元，使用`glActiveTexture`即可激活贴图单元
			- ```cpp
			  glActiveTexture(GL_TEXTURE0); // activate the texture unit first before binding texture
			  glBindTexture(GL_TEXTURE_2D, texture);
			  ```
			- `glBindTexture`实际上是将贴图绑定到当前激活的Texture Unit上
			- 不过`GL_TEXTURE0`永远是默认激活的，因此如果在`glBindTexture`前没有调用`glActiveTexture`，则都是默认绑定到该贴图单元上
		- OpenGL要求至少有**[[$red]]==16==个贴图单元**，从`GL_TEXTURE0`到`GL_TEXTURE15`
			- 不同的贴图单元之间可以通过加减整数来访问，例如：`GL_TEXTURE0+8=GL_TEXTURE8`，这在需要循环处理不同贴图单元时很实用
		-
- # Transformation
  collapsed:: true
	- 这一小节主要复习了一下最基本的线性代数变换，参见[[第二课：变换]]
	- 在OpenGL相关编程的实践中，这些变换操作基本都由**[[$red]]==GLM==**库完成
		- [下载连接](https://glm.g-truc.net/0.9.9/index.html)
- # Coordinate Systems
  collapsed:: true
	- 这一节主要讲的是不同坐标系之间的转换，主要还是[[第二课：变换]]中的内容
	- 生成透视投影变换坐标系使用`glm::perspective函数`，例如：
		- ``glm::mat4 proj = glm::perspective(glm::radians(45.0f), (float)width/(float)height, 0.1f, 100.0f);``
		- 第一个参数是fov，最后两个参数分别是近平面和远平面的距离
	- **Clip**空间是在进行光栅化之前最后在3D空间的坐标，也就是在经过了MVP变换之后的顶点坐标
		- $V_{clip} = M_{projection} \cdot M_{view} \cdot M_{model}\cdot V_{local}$
	- **Vertex Shader [[$red]]==最终输出的顶点坐标就应当在clip space 下==**
- # Camera
  collapsed:: true
	- ## 复习：定义一个相机的四要素
		- 这四个要素都可以通过一个向量表示
		- ![image.png](../assets/image_1746945848844_0.png){:height 165, :width 719}
		- ### Position
			- 相机所在的位置
		- ### Direction
			- 相机所看向的方向，但在实际运用中，往往和看向物体的方向**[[$red]]==相反==**
				- 因为基本上所有的图形api，相机都是看向z轴负方向
			- 实际定义：
				- ```c++
				  glm::vec3 cameraTarget = glm::vec3(0.0f, 0.0f, 0.0f);
				  glm::vec3 cameraDirection = glm::normalize(cameraPos - cameraTarget);
				  ```
				- 注意这里就能看出来，计算出来的Direction方向**[[$red]]==是从目标指向相机==**，而不是从相机指向目标
		- ### Right Axis
			- 相机的右向轴
			- 通过叉乘可以得到
			- ```C++
	...
	- ## Look At Transition Matrix
		...
		- 用于进行Model变换的矩阵，其中：
			- $\color{red}{R}$是Right Vector
			- $\color{green}{U}$是Up Vector
			- $\color{blue}{D}$是Direction Vector
			- $\color{purple}{P}$是Position Vector
		- 幸运的是，在实际操作中不需要手动计算$LookAt$矩阵，opengl有现成的库函数可以计算得出该矩阵：
			- ```cpp
			  glm::mat4 view;
