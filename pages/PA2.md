...
- # 补充
	...
	- ## C在栈上分配的空间
		- 每一次函数调用都会在栈上划分一定的空间
		- 本地变量都是分配在这个栈空间上
		- 因此对于一个本地的数组，**有可能每次调用该函数的时候，[[$red]]==都位于同一片物理地址。==**
			- **所以初始化很重要**，例如声明一个字符串，使用前建议把首字符设置为``\0``
			- 没有初始化，有可能导致直接使用上一次调用的数据。
	- ## C和C++的混编
	...
	- ## 定义是如何从Kconfig最终到c文件中的？
		- Make会调用menuconfig目标
		- 该目标通过NEMU框架中自带的tool kconfig从Kconfig文件生成图形化菜单
			- 这个kconfig的实现用到了ncurses库，这个库提供构建虚拟终端中**类GUI**的API
	...
	- ## ELF文件中的符号表
		...
		- 位于文件的``.symtab``段
	- ## 什么时候getopt-long会返回1？
		- 在指定optstring的时候，如果在开头加一个``-``，则处理到所有的非option参数，返回值会是1
		- 似乎只会对遇到的第一个不匹配参数有用(在处理到第一个非option的参数之后会结束处理，忽略所有此参数后的所有输入)
- # 部分问题回答
